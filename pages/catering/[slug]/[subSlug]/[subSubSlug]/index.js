...
const SubSubCateringPage = ({ data, response, tax }) => {
...
export default SubSubCateringPage;
...
export async function getStaticPaths(){
    ...
    data?.cateringPages?.nodes && data?.cateringPages?.nodes.map( page => {
        data?.cateringPages?.nodes && page?.children?.nodes?.map(child => {
            page?.children?.nodes && child?.childChild?.nodes?.map(grandChild => {
                if(! isEmpty( grandChild?.uri )){
                    pathsData.push({
                        params: {
                            slug: page.slug,
                            subSlug: child.slug,
                            subSubSlug: grandChild.slug
                        }
                    })
                }
            })
